#!/usr/bin/env python3
"""
Hui Memory Compression Orchestrator
...
# Importing this module directly (tests, libraries) bypasses cli.py's UTF-8
# stream setup. Keep diagnostics printable on legacy Windows code pages.
...
def backup_dir_for(filepath: Path) -> Path:
    ...
    `.original.md` copies as live files. Base dir is platform-aware:
      - Windows: %LOCALAPPDATA%\\hui-compress\\backups
      - else:    $XDG_DATA_HOME/hui-compress/backups if set,
                 else ~/.local/share/hui-compress/backups
    ...
    if os.name == "nt" or sys.platform == "win32":
        ...
        base = base / "hui-compress" / "backups"
    else:
    ...
    if api_key:
        try:
            ...
            msg = client.messages.create(
                model=os.environ.get("HUI_MODEL", "claude-sonnet-4-5"),
                max_tokens=8192,
...
def build_compress_prompt(original: str) -> str:
    return f"""
Compress this markdown into hui format.
...
def build_fix_prompt(original: str, compressed: str, errors: List[str]) -> str:
    ...
    return f"""You are fixing a hui-compressed markdown file. Specific validation errors were found.

CRITICAL RULES:
- DO NOT recompress or rephrase the file
- ONLY fix the listed errors — leave everything else exactly as-is
- The ORIGINAL is provided as reference only (to restore missing content)
- Preserve hui style in all untouched sections
...
def changed_prose_blocks(base: str, current: str) -> list[tuple[int, int, str]]:
    ...
    Conservative boundaries keep headings, fenced code and blank separators intact.
    Returned offsets refer to ``current`` and replacements are applied in reverse.
    """
...
def compress_file(filepath: Path) -> bool:
    ...
    if backup_path.exists():
        status(f"⚠️ Backup file already exists: {backup_path}")
    ...
    if compressed_body is None or not compressed_body.strip():
        status("❌ Compression aborted: Claude returned an empty response.")
    ...
    if compressed_body.strip() == body.strip():
        status("❌ Compression aborted: output is identical to input.")
        print("   Likely causes: Claude refused, returned the prompt verbatim, or the file is")
        print("   already in hui form. Original file is untouched (no backup created).")
        return False
    ...
    if backup_readback != original_text:
        status(f"❌ Backup write verification failed: {backup_path}")
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if attempt == MAX_RETRIES - 1:
            ...
            backup_path.unlink(missing_ok=True)
            status("❌ Failed after retries — original restored")
            return False
