...
Hui like simple. Small focused PR > big rewrite.

---

## Quick orientation

The repo distributes one skill (hui) plus a handful of sub-skills
(hui-commit, hui-review, hui-compress, huicrew-*) to many
...
copies live under `plugins/hui/` and similar mirror dirs — those are
...
|---|---|
| Hui behavior (intensity levels, voice, rules) | `skills/hui/SKILL.md` |
| Hui commit-message format | `skills/hui-commit/SKILL.md` |
| Hui code-review format | `skills/hui-review/SKILL.md` |
| Hui compress logic | `skills/hui-compress/SKILL.md` and `skills/hui-compress/scripts/` |
| Hui quick-reference card | `skills/hui-help/SKILL.md` |
| Huicrew decision guide (when to delegate to subagents) | `skills/huicrew/SKILL.md` |
| huicrew subagent definitions | `agents/huicrew-investigator.md`, `agents/huicrew-builder.md`, `agents/huicrew-reviewer.md` |
| Auto-activation rule body (Cursor/Windsurf/Cline/Copilot) | `src/rules/hui-activate.md` |
| Add support for a new agent | `bin/install.js` (PROVIDERS array) |
| Per-repo init script (drops rule files into a user's repo) | `src/tools/hui-init.js` |
| Claude Code hooks | `src/hooks/hui-activate.js`, `src/hooks/hui-mode-tracker.js`, `src/hooks/hui-config.js`, `src/hooks/hui-statusline.sh`, `src/hooks/hui-statusline.ps1` |
| Settings.json read/write helpers | `bin/lib/settings.js` |
| MCP shrink server | `src/mcp-servers/hui-shrink/` |
...
4. If the skill should be in the Claude Code plugin, add a sync step to `.github/workflows/sync-skill.yml` so CI mirrors it into `plugins/hui/skills/<name>/`.
...
- **Mention the CI sync.** If you edited a source-of-truth file, note it: "CI will resync `plugins/hui/skills/...` on merge."

PR descriptions don't need to be long. Hui style fine. Just say what change, why.
...
- **Hooks must silent-fail on filesystem errors.** A `try/catch` that swallows the error is correct here. A hook that throws blocks Claude Code session start — that's user-facing breakage. See existing patterns in `src/hooks/hui-activate.js`.
...
- **Symlink-safe flag writes via `safeWriteFlag()`** in `src/hooks/hui-config.js`. The flag file lives at a predictable path under `$CLAUDE_CONFIG_DIR/`; without `O_NOFOLLOW` and a parent-symlink check, a local attacker can clobber any file the user can write.
...
Hui like contribution. You bring rock, hui put rock in pile. Pile
