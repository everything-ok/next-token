...
| `hui --update` | 已装→`npm install -g`(拉最新包)+ `claude plugin update hui@hui` +`refreshing standalone hooks` + `update done`；保留 `.hui-active` | ✅ |
...
- `1.2.4`（2026-07-30）：修复 `--update` 的 `claude plugin update` 参数——用 `hui@hui`（marketplace@plugin id）而非裸名 `hui`（报 "Plugin hui not found"）。install/uninstall 路径本就用 `hui@hui`,update 漏了。加裸名 fallback 兼容老 CLI。实测 `hui --update` 全绿无 failed。重算 `skills-lock.json` 修复 CRLF 漂移。
