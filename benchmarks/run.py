#!/usr/bin/env python3
"""Benchmark hui vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "hui" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, hui_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("hui", hui_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        hui_medians = statistics.median(
            [t["output_tokens"] for t in entry["hui"]]
        )
        savings = 1 - (hui_medians / normal_medians) if normal_medians > 0 else 0
    ...
    avg_hui = round(statistics.mean([r["hui_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Hui (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['hui_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_hui']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark hui vs normal Claude")
    ...
    results = run_benchmarks(client, args.model, prompts, hui_system, args.trials)
