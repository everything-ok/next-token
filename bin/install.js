#!/usr/bin/env node
// hui — unified cross-platform installer.
//
...
const MCP_SHRINK_PKG = 'hui-shrink';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // through to the switch and is rejected — hui-shrink is a proxy
    ...
    switch (a) {
      ...
      case '--with-mcp-shrink': {
        ...
        if (v && !v.startsWith('--')) {
          ...
          die('error: --with-mcp-shrink requires an upstream command — hui-shrink\n' +
  ...
  //   • withMcpShrink — hui-shrink is a proxy that needs an upstream
...
function checkWslWindowsNode() {
  ...
  if (process.env.WSL_DISTRO_NAME) {
    die('hui: detected Windows Node.js running inside WSL.\n' +
...
function checkNodeVersion() {
  ...
  if (major < 18) die(`hui: Node ${process.versions.node} too old. Need Node ≥18. https://nodejs.org`);
}
...
async function installClaude(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /hui/i.test(r.stdout || '')) alreadyInstalled = true;
  }
  let pluginInstallSucceeded = false;
  if (alreadyInstalled) {
    note('  hui plugin already installed (use --force to reinstall)');
    ...
    // Dry runs must not create configDir/tmp or otherwise write to disk.
  ...
  // hui-*.js scripts, so Claude Code crashes every SessionStart /
  ...
  // skip standalone wiring because the plugin manifest handles hooks.
  {
    ...
    if (settings) {
      ...
      if (pruned > 0) {
        note(`  removed ${pruned} orphaned hui hook entr${pruned === 1 ? 'y' : 'ies'} from settings.json (target script missing)`);
        if (!opts.dryRun) {
  ...
  // event (two HUI MODE blocks, two reinforcement lines).
  let shouldWireHooks;
  ...
  if (opts.withMcpShrink) {
    say('  → wiring hui-shrink MCP proxy (--with-mcp-shrink)');
    const r = installMcpShrink(ctx);
    if (r.kind === 'ok')   results.installed.push('hui-shrink');
    if (r.kind === 'skip') results.skipped.push(['hui-shrink', r.why]);
    if (r.kind === 'fail') results.failed.push(['hui-shrink', r.why]);
  }
...
function installGemini(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /hui/i.test(r.stdout || '')) {
      note('  hui extension already installed (use --force to reinstall)');
...
function installHermes(ctx) {
  ...
  if (!repoRoot) {
    warn('  Hermes native install requires a local clone of the hui repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd next-token && node bin/install.js --only hermes');
...
const OPENCODE_AGENTS_MD_BEGIN = '<!-- hui-begin -->';
const OPENCODE_AGENTS_MD_END = '<!-- hui-end -->';
...
function installOpencode(ctx) {
  ...
  if (!repoRoot) {
    warn('  opencode native install requires a local clone of the hui repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd next-token && node bin/install.js --only opencode');
  ...
  const pluginDir   = path.join(dir, 'plugins', 'hui');
  ...
  if (opts.dryRun) {
    ...
    note(`  would copy plugin.js + package.json + hui-config.cjs into ${pluginDir}/`);
    note(`  would copy ${OPENCODE_COMMAND_FILES.length} command files into ${commandsDir}/`);
    note(`  would copy ${OPENCODE_AGENT_FILES.length} huicrew agents into ${agentsDir}/`);
    note(`  would copy ${OPENCODE_SKILL_DIRS.length} skill dirs into ${skillsDir}/`);
    note(`  would patch ${opencodeJson} with "plugin" entry${opts.withMcpShrink ? ' + hui-shrink MCP' : ''}`);
  ...
  try {
    // 1. Plugin dir — copy plugin.js, package.json, hui-config.js (sibling).
    ...
    const pluginPayload = [
      ...
      [path.join(repoRoot, 'src', 'hooks', 'hui-config.js'),
       path.join(pluginDir, 'hui-config.cjs')],
    ];
    ...
    const ruleBody = fs.readFileSync(path.join(repoRoot, 'src', 'rules', 'hui-activate.md'), 'utf8').trimEnd() + '\n';
    ...
    if (fs.existsSync(agentsMd)) {
      ...
      if (alreadyFenced) {
        note(`  ${agentsMd} already contains hui ruleset`);
      } else if (alreadyByLegacySentinel) {
        if (!opts.force) {
          note(`  ${agentsMd} contains a legacy (un-fenced) hui block — leaving as-is`);
    ...
    // 6. opencode.json — add plugin entry; optional hui-shrink MCP.
    ...
    if (opts.withMcpShrink) {
      ...
      // produced. hui-shrink is a proxy — it crashes without an upstream,
      // so we always wire one through.
      if (!cfg.mcp || typeof cfg.mcp !== 'object') cfg.mcp = {};
      if (!cfg.mcp['hui-shrink']) {
        cfg.mcp['hui-shrink'] = {
          type: 'local',
          command: ['npx', '-y', MCP_SHRINK_PKG, ...opts.withMcpShrink],
          enabled: true,
        };
        process.stdout.write(`  registered hui-shrink MCP server (wraps: ${opts.withMcpShrink.join(' ')})\n`);
      }
    }
...
// Drops skills/hui/ into the OpenClaw workspace and appends a small
...
// hui discoverable via `openclaw skills list`. See bin/lib/openclaw.js
...
async function installHooks(ctx) {
  ...
  SETTINGS.addCommandHook(settings, 'SessionStart', {
    ...
    marker: 'hui-activate',
    timeout: 5,
    statusMessage: 'Loading hui mode...',
  });
  ...
  const slCmd = IS_WIN
    ? `${psHost} -NoProfile -ExecutionPolicy Bypass -File "${path.join(hooksDir, 'hui-statusline.ps1')}"`
    : `bash "${statusline}"`;
  if (!settings.statusLine) {
    ...
    if (existing.includes(statusline) || existing.includes('hui-statusline')) {
      process.stdout.write('  statusline badge already configured.\n');
    } else {
      process.stdout.write('  NOTE: existing statusline detected — hui badge NOT added.\n');
...
function installMcpShrink(ctx) {
  ...
  // gets `npx -y hui-shrink <upstream tokens...>` so it has something
  // to wrap.
  const upstream = opts.withMcpShrink;
  const r = runSpawn(
    'claude',
    ['mcp', 'add', 'hui-shrink', '--', 'npx', '-y', MCP_SHRINK_PKG, ...upstream],
    null, opts.dryRun
  );
  if (spawnOk(r)) {
    note(`    registered, wrapping: ${upstream.join(' ')}`);
    note(`    Edit ~/.claude.json mcpServers["hui-shrink"] to change the upstream,`);
    note('    or `claude mcp remove hui-shrink` to drop it.');
    note(`    Docs: https://github.com/${REPO}/tree/main/src/mcp-servers/hui-shrink`);
    return { kind: 'ok' };
  }
...
async function runInit(ctx) {
  ...
  const local = repoRoot && path.join(repoRoot, 'src/tools/hui-init.js');
...
async function loadRemoteHookChecksums() {
  const tmp = path.join(os.tmpdir(), `hui-checksums-${process.pid}-${Date.now()}.sha256`);
  try {
...
function uninstall(ctx) {
  ...
  if (fs.existsSync(settingsPath)) {
    const settings = SETTINGS.readSettings(settingsPath);
    if (settings) {
      const removed = SETTINGS.removeHuiHooks(settings);
      ...
      if (settings.statusLine) {
        ...
        if (cmd.includes('hui-statusline')) delete settings.statusLine;
      }
      SETTINGS.validateHookFields(settings);
      if (!opts.dryRun) SETTINGS.writeSettings(settingsPath, settings);
      ok(`  removed ${removed} hui hook entr${removed === 1 ? 'y' : 'ies'} from settings.json`);
    }
  }
  ...
  // machine where hui was never installed (or was already removed) doesn't
  ...
  if (hasCmd('claude')) {
    ...
    if (probe.status === 0 && /hui/i.test(probe.stdout || '')) {
      const r = runSpawn('claude', ['plugin', 'uninstall', 'hui@hui'], null, opts.dryRun);
  ...
  if (hasCmd('gemini')) {
    ...
    if (probe.status === 0 && /hui/i.test(probe.stdout || '')) {
      runSpawn('gemini', ['extensions', 'uninstall', 'hui'], null, opts.dryRun);
    } else {
  ...
  const ocPluginDir = path.join(ocDir, 'plugins', 'hui');
  if (fs.existsSync(ocPluginDir)) {
    ...
    if (fs.existsSync(ocJson)) {
      ...
      if (cfg) {
        ...
        if (cfg.mcp && typeof cfg.mcp === 'object' && cfg.mcp['hui-shrink']) {
          delete cfg.mcp['hui-shrink'];
  ...
  if (fs.existsSync(path.join(ocwWs, 'skills', 'hui')) || fs.existsSync(path.join(ocwWs, 'SOUL.md'))) {
    const log = {
      write: (s) => process.stdout.write(s),
      note: (s) => note(s),
      warn: (s) => warn(s),
    ...
    if (r.touched) ok('  pruned hui entries from OpenClaw workspace');
  }
...
// ── Migrate-from-hui ────────────────────────────────────────────────────────
// Convert a stale standalone hook install into the plugin-managed layout.
// Read-only unless --force: detects orphaned managed hooks and proposes the
// exact repair (prune + re-wire). Never touches user-authored hooks or rules.
function migrateFromHui(opts, configDir) {
  const settingsPath = path.join(configDir, 'settings.json');
  const settings = SETTINGS.readSettings(settingsPath);
...
// ── Doctor ─────────────────────────────────────────────────────────────────
// Read-only diagnostic. Reports node, npm, detected providers, and the state of
// HUI resources under configDir. Never writes. --json emits machine output.
function doctor(opts, configDir) {
  const report = { product: PRODUCT_NAME, npmPackage: NPM_PACKAGE_NAME, repository: REPOSITORY, checks: {} };
  ...
  const npmProbe = spawnXplat(npmBin, ['--version'], { encoding: 'utf8' });
  report.checks.npm = {
    ok: spawnOk(npmProbe),
    version: (npmProbe.stdout || '').trim() || null,
  };

  report.checks.configDir = { path: configDir, exists: fs.existsSync(configDir) };
  ...
  if (settings) {
    ...
    if (ss > 1 || ups > 1) hookState.duplicateManaged = Math.max(ss, ups);
  }
  report.checks.claudeHooks = hookState;

  const flagPath = path.join(configDir, '.hui-active');
  report.checks.activeFlag = { exists: fs.existsSync(flagPath), mode: fs.existsSync(flagPath) ? fs.readFileSync(flagPath, 'utf8').trim() : null };

  const pluginsDir = path.join(configDir, 'plugins');
  report.checks.plugin = { path: pluginsDir, exists: fs.existsSync(pluginsDir) };

  const detected = PROVIDERS.filter(p => detectMatch(p.detect)).map(p => ({ id: p.id, label: p.label, mech: p.mech }));
  report.checks.detectedAgents = detected;

  const repoRoot = detectRepoRoot();
  report.checks.localClone = { detected: !!repoRoot, path: repoRoot };

  // Stale-install heuristics: managed hook commands whose target script no longer resolves.
  const stale = [];
  if (settings) {
    const pruned = SETTINGS.pruneOrphanedManagedHooks(settings, configDir);
    if (pruned > 0) stale.push({ kind: 'orphanedManagedHooks', count: pruned, note: 'settings.json references HUI hook scripts that no longer exist on disk' });
  }
  report.checks.stale = stale;

  const problems = [];
  if (!report.checks.node.ok) problems.push('node missing');
  if (!report.checks.npm.ok) problems.push('npm missing');
  if (hookState.duplicateManaged) problems.push(`duplicate managed hooks (${hookState.duplicateManaged})`);
  for (const s of stale) problems.push(s.kind);
...
function printDoctorHuman(r) {
  const ok = (b) => b ? '✓' : '✗';
  process.stdout.write(`${PRODUCT_NAME} doctor — ${r.repository}\n\n`);
  process.stdout.write(`${ok(r.checks.node.ok)} node ${r.checks.node.version} (${r.checks.node.path})\n`);
  process.stdout.write(`${ok(r.checks.npm.ok)} npm ${r.checks.npm.version || 'not found'}\n`);
  process.stdout.write(`${ok(r.checks.configDir.exists)} config dir: ${r.checks.configDir.path}\n`);
  ...
  process.stdout.write(`${ok(f.exists)} active flag${f.exists ? ` (mode: ${f.mode})` : ''}\n`);
  process.stdout.write(`${ok(r.checks.plugin.exists)} plugin dir: ${r.checks.plugin.path}\n`);
  process.stdout.write(`${ok(r.checks.localClone.detected)} local clone${r.checks.localClone.detected ? ` (${r.checks.localClone.path})` : ''}\n`);
  process.stdout.write(`\nDetected agents (${r.checks.detectedAgents.length}):\n`);
  for (const a of r.checks.detectedAgents) process.stdout.write(`  • ${a.id} — ${a.label} (${a.mech})\n`);
  if (r.checks.stale.length) {
    process.stdout.write(`\nStale / needs attention:\n`);
    for (const s of r.checks.stale) process.stdout.write(`  • ${s.kind}${s.count ? ` (×${s.count})` : ''}: ${s.note}\n`);
  }
  process.stdout.write(`\n${r.ok ? '✓ all checks passed' : '✗ ' + r.problems.length + ' problem(s): ' + r.problems.join(', ')}\n`);
}

// ── Help ───────────────────────────────────────────────────────────────────
function printHelp() {
  process.stdout.write(`${PRODUCT_NAME} installer — product name: ${PRODUCT_NAME}; npm distribution: ${NPM_PACKAGE_NAME}.

USAGE
  npx -y ${NPM_PACKAGE_NAME} -- [flags]
...
FLAGS
  ...
  --with-mcp-shrink="<upstream cmd>"
                        Claude Code (and opencode): register hui-shrink MCP
                        proxy wrapping the given upstream. Default OFF.
                        hui-shrink crashes without an upstream, so a value
  ...
  --uninstall, -u       Remove HUI from this machine.
  --doctor              Diagnose Node/npm, detected agents, plugin/hooks/skills
                        status, and stale installs. Read-only — writes nothing.
...
EXAMPLES
  npx -y ${NPM_PACKAGE_NAME}                        # default install
...
async function main() {
  ...
  if (opts.doctor) { return doctor(opts, configDir); }
  if (opts.migrateFromHui) { return migrateFromHui(opts, configDir); }
  ...
  if (opts.withInit) {
    ...
    if (await runInit(ctx)) ctx.results.installed.push(`hui-init (${process.cwd()})`);
    else                    ctx.results.failed.push(['hui-init', 'src/tools/hui-init.js failed']);
    process.stdout.write('\n');
