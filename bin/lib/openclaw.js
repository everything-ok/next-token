// hui → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make hui always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/hui/SKILL.md into <workspace>/skills/hui/
...
// hui SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'hui-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/hui-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    '## Hui mode (always on)',
    '',
    ...
    '',
    "The full ruleset and intensity levels live in this workspace's hui skill:",
    '',
    '  skills/hui/SKILL.md',
    '',
    'Default intensity: `full`. Switch with `/hui lite|full|ultra|wenyan`.',
    'Stop with: "stop hui" / "normal mode" / "deactivate hui".',
    '',
    'Auto-Clarity: drop hui for security warnings, irreversible action',
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'hui', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (hui bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains hui bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip hui block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped hui block from ${soulFile}`);
        touched = true;
      }
    }
  }
