// hui — JSONC-tolerant settings.json read/write + defensive hook validation.
//
// Lifted in spirit from gsd-build/get-shit-done's stripJsonComments + readSettings.
// Reused by bin/install.js and (optionally) by hooks/hui-activate.js so a
...
//   hasHuiHook(settings, ev)   → idempotency probe
//   addCommandHook(settings, ev, opts) → no-op if substring marker already present
//   removeHuiHooks(settings)   → uninstall helper
//
...
function readSettings(p) {
  ...
  try { raw = fs.readFileSync(p, 'utf8'); }
    ...
    process.stderr.write(`hui: cannot read ${p}: ${e.message}\n`);
    return null;
  }
  if (!raw.trim()) return {};
  ...
  try { return JSON.parse(stripJsonComments(raw)); }
    ...
    process.stderr.write(`hui: warning — ${p} is not valid JSON or JSONC: ${e.message}\n`);
    return null;
  }
}
...
// ── Idempotency probe ──────────────────────────────────────────────────────
function hasHuiHook(settings, event, marker = 'hui') {
...
function addCommandHook(settings, event, opts) {
  ...
  if (hasHuiHook(settings, event, marker)) return false;
...
// against a bare "hui" substring, which also matches user-authored hooks
...
// match — not substring — so `myhui-activate.js` or a user hook living
// under a `hui-notes/` directory is never treated as ours. win32.basename
...
function removeHuiHooks(settings) {
...
// on user-authored hooks that just happen to mention "hui".
...
// hui-*.js) to the Claude Code plugin disables/renames those local
...
// …hui-activate.js` (issue #471). rewriteLegacyManagedHookCommands can't
// help — it only matches the bare-node shape and these orphans are usually
// absolute-node — and removeHuiHooks runs only on uninstall.
//
...
function pruneOrphanedManagedHooks(settings, configDir) {
  ...
  // `myhui-activate.js` is never touched) and that resolved path is absent.
