# hui — installer shim (Windows / PowerShell).
#
...
function Install-Hui {
  ...
  if (-not $node) {
    Write-Error @"
hui: Node.js (>=18) required. Install:
...
"@
  ...
  if ($nodeMajor -lt 18) {
    Write-Error "hui: Node $nodeMajor too old. Need Node >=18. Upgrade: https://nodejs.org"
    exit 1
  }
  ...
  if (-not $npx) {
    Write-Error "hui: npx required (ships with Node >=18). Reinstall Node.js."
    exit 1
  }
