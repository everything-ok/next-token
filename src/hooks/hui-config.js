#!/usr/bin/env node
// hui — shared configuration resolver
//
// Resolution order for default mode:
//   1. HUI_DEFAULT_MODE environment variable
...
//      - $XDG_CONFIG_HOME/hui/config.json (any platform, if set)
//      - ~/.config/hui/config.json (macOS / Linux fallback)
//      - %APPDATA%\hui\config.json (Windows fallback)
//   4. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'hui');
  }
  ...
  return path.join(os.homedir(), '.config', 'hui');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.HUI_DEFAULT_MODE;
...
// flag path (~/.claude/.hui-active) with a symlink to clobber other files.
//
...
//
// Set HUI_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.HUI_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[hui] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[hui] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[hui] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.hui-active.${process.pid}.${Date.now()}`);
...
// ($CLAUDE_CONFIG_DIR/.hui-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[hui] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
...
// append {ts, mode, prev} to $CLAUDE_CONFIG_DIR/.hui-mode-log.jsonl so
// hui-stats can attribute output tokens to the mode that was active when
// each message was generated, instead of whatever mode the flag holds at
// stats time. mode/prev are a VALID_MODES string or null (null = hui off).
...
const MODE_LOG_BASENAME = '.hui-mode-log.jsonl';

function recordModeChange(claudeDir, newMode) {
  try {
    const current = readFlag(path.join(claudeDir, '.hui-active'));
