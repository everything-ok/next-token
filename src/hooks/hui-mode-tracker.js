...
// Inlined from src/hui-command-parser.js so this hook is self-contained when
// copied to ~/.claude/hooks/ by the installer. The installer's HOOK_FILES list
// ships only src/hooks/* — it does NOT ship src/hui-command-parser.js — so a
// require('../hui-command-parser') throws MODULE_NOT_FOUND at load time in the
// standalone-hook layout and the hook exits 1 on every UserPromptSubmit. The
// plugin path (run from ${CLAUDE_PLUGIN_ROOT}/src/hooks/) keeps working too.
// tests/test_hui_command_parser.js asserts this copy stays in sync with the
// canonical src/hui-command-parser.js module.
const PARSE_LEVELS = new Set(['lite', 'full', 'ultra', 'wenyan', 'wenyan-lite', 'wenyan-ultra']);
...
function parseHuiCommand(raw) {
  ...
  if (PARSE_LEVELS.has(args[0]) || args[0] === 'wenyan-full') {
...
function modeForParsedCommand(command, defaultMode) {
  ...
  if (command.action === 'on') return PARSE_LEVELS.has(command.args[0]) ? command.args[0] : null;
  if (command.action === 'mode') return PARSE_LEVELS.has(command.args[0]) ? command.args[0] : null;
