#!/usr/bin/env node
// hui-stats — read the active Claude Code session log, print real token
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, VALID_MODES, MODE_LOG_BASENAME } = require('./hui-config');
...
// Rule injection costs input tokens every active turn. Conservative documented
// midpoint; net results stay estimates until per-model prompt accounting exists.
const RULE_INPUT_TOKENS_PER_TURN = 1200;
...
// Detect *.original.md / *.md pairs left behind by hui-compress. The
...
// estimate (verbose tokens counted as compressed) or zero it (hui tokens
// counted as uncompressed). The mode tracker + SessionStart hook append
// {ts, mode, prev} rows to .hui-mode-log.jsonl on every actual transition;
...
// a mode string or 'none' (hui inactive).
...
function deriveSavings({ byMode, model, turns = 0 }) {
  ...
  const estimatedInputOverhead = Math.max(0, turns) * RULE_INPUT_TOKENS_PER_TURN;
  const estNetTokens = estSavedTokens - estimatedInputOverhead;
  const price = priceForModel(model);
  const estSavedUsd = price !== null ? (estSavedTokens / 1_000_000) * price : 0;
  return { estSavedTokens, estSavedUsd, estimatedInputOverhead, estNetTokens };
}
...
// would-be OUTPUT tokens that hui avoided. That is the only ratio we can
...
// Pro/Max limits, and are not reduced by hui, so real limit relief is far
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nHui Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /hui-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model, attribution }) {
  if (turns === 0) {
    return '🪨 hui armed but no turns yet — hui.sh';
  }
  const attr = attribution || wholeSessionAttribution(mode, outputTokens);
  const { estSavedTokens, estSavedUsd } = deriveSavings({ byMode: attr.byMode, model, turns });

  if (estSavedTokens > 0) {
    const usd = estSavedUsd > 0 ? ` (~${formatUsd(estSavedUsd)})` : '';
    return `🪨 Saved ${estSavedTokens.toLocaleString()} output tokens${usd} across ${turns} turns this session — hui.sh`;
  }
  return `🪨 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — hui.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed, attribution }) {
  ...
  if (!uniform) {
    ...
    for (const key of activeKeys) {
      ...
      const label = key === 'none' ? 'hui off' : key;
      const note = r != null
    ...
    const { estSavedTokens: estSaved, estSavedUsd, estimatedInputOverhead, estNetTokens } =
      deriveSavings({ byMode: attr.byMode, model, turns });
    const estNormal = estSaved + outputTokens;
    const usdLine = estSavedUsd > 0 ? `Est. saved (USD):      ~${formatUsd(estSavedUsd)}\n` : '';
    const netLabel = estNetTokens > 0 ? 'net output-input estimate' : estNetTokens < 0 ? 'net overhead estimate' : 'net neutral estimate';
    footer = 'Savings est. from benchmark output compression. Input overhead uses ~1.2k injected-rule tokens per turn; cache/input billing varies by provider.';
    if (price !== null) footer += ` Pricing for ${model}.`;
    footer += ' output tokens only; input/cache usage is unchanged outside the rule-input estimate.';
    savings = (`Est. without hui:  ${estNormal.toLocaleString()}\n` +
              `Est. tokens saved:     ${estSaved.toLocaleString()} (~${Math.round(ratio * 100)}% of output)\n` +
              `Est. rule input cost:  ~${estimatedInputOverhead.toLocaleString()}\n` +
              `Est. net token effect: ${estNetTokens >= 0 ? '+' : ''}${estNetTokens.toLocaleString()} (${netLabel})\n` +
              usdLine).replace(/\n$/, '');
  } else if (mode && mode !== 'off') {
    savings = `No savings estimate for '${mode}' mode — only 'full' has benchmark data.`;
  } else {
    savings = 'Hui not active this session.';
  }
...
function main() {
  ...
  // /hui-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /hui-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .hui-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏  ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.hui-statusline-suffix'), suffix);
  }
