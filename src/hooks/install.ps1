# hui — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/2454760302hui/next-token/main/src/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the hui hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/2454760302hui/next-token/main/src/hooks"
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("hui")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasHuiHook "SessionStart") -and (& $hasHuiHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Hui hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "hui-activate.js"))) {
    Write-Host "Reinstalling hui hooks (-Force)..."
} else {
    Write-Host "Installing hui hooks..."
}
...
$tmpScript = Join-Path $env:TEMP "hui-install-$([System.Diagnostics.Process]::GetCurrentProcess().Id).js"
try {
...
Write-Host "  - SessionStart hook: auto-loads hui rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/hui lite, /hui ultra, /hui-commit, etc.)"
Write-Host "  - Statusline badge: shows [HUI] or [HUI:ULTRA] etc."
