#!/bin/bash
# hui — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash src/hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/2454760302hui/next-token/main/src/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the hui hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/2454760302hui/next-token/main/src/hooks"

HOOK_FILES=("package.json" "hui-config.js" "hui-activate.js" "hui-mode-tracker.js" "hui-stats.js" "hui-statusline.sh" "huicrew-model-overrides.js")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if HUI_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.HUI_SETTINGS, 'utf8'));
      const hasHuiHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('hui'))
        );
      process.exit(
        hasHuiHook('SessionStart') &&
        hasHuiHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Hui hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/hui-activate.js" ]; then
  echo "Reinstalling hui hooks (--force)..."
else
  echo "Installing hui hooks..."
fi
...
chmod +x "$HOOKS_DIR/hui-statusline.sh"
...
HUI_SETTINGS="$SETTINGS" HUI_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.HUI_SETTINGS;
  const hooksDir = process.env.HUI_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/hui-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load hui rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/hui-activate.js\"',
        timeout: 5,
  ...
  // UserPromptSubmit — track mode changes when user types /hui commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/hui-mode-tracker.js\"',
        timeout: 5,
  ...
  // Statusline — wire hui badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — hui badge NOT added.');
...
echo "  - SessionStart hook: auto-loads hui rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/hui lite, /hui ultra, /hui-commit, etc.)"
echo "  - Statusline badge: shows [HUI] or [HUI:ULTRA] etc."
