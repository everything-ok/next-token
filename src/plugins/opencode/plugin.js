// hui — opencode plugin
//
// Provides dynamic hui mode tracking for opencode:
// - Writes the mode flag on each session start (via the `event` dispatcher)
// - Parses user messages for /hui commands and natural-language toggles
// - Injects per-turn reinforcement into the system prompt
//
// Bun ESM module; loads the existing security-hardened helpers from
// hui-config.js via createRequire so the symlink-safe flag-write code
// lives in one place.
//
// Layout once installed:
//   ~/.config/opencode/plugins/hui/
//   ├── package.json
//   ├── plugin.js              ← this file
//   └── hui-config.cjs     ← copied sibling of src/hooks/hui-config.js
//
// The always-on hui ruleset is provided separately via
...
// https://github.com/everything-ok/next-token/issues/418
// https://github.com/everything-ok/next-token/issues/421
...
// When installed: hui-config.cjs sits next to plugin.js (copied by
...
// src/hooks/hui-config.js, which lives in a directory whose own
...
// hui-config needs (fs/path/os).
function loadConfig() {
  const installed = join(here, 'hui-config.cjs');
  const dev = join(here, '..', '..', 'hooks', 'hui-config.js');
...
// Modes handled by independent skills — not selectable via /hui <arg>.
...
const flagPath = path.join(opencodeConfigDir(), '.hui-active');

function reinforcementLine(mode) {
  return 'HUI MODE ACTIVE (' + mode + '). ' +
...
// null when the prompt doesn't change state. Mirrors hui-mode-tracker.js.
function parseModeChange(promptRaw) {
  ...
  // literal quote characters ("/hui ultra"\n) — unwrap symmetric quotes
  ...
  // like hui" doesn't trip the activation regex.
  if (/\b(stop|disable|deactivate|turn off)\b.*\bhui\b/i.test(prompt) ||
      /\bhui\b.*\b(stop|disable|deactivate|turn off)\b/i.test(prompt) ||
      /\bnormal mode\b/i.test(prompt)) {
    return 'off';
  }

  // Expanded /hui command template. opencode replaces a typed
  // "/hui <level>" with the command file's body ("Activate hui
  ...
  const tpl = /^activate hui mode:[ \t]*(\S*)/.exec(prompt);
  if (tpl) {
    const arg = tpl[1] || '';
  ...
  if (/\b(activate|enable|turn on|start|talk like)\b.*\bhui\b/i.test(prompt) ||
      /\bhui\b.*\b(mode|activate|enable|turn on|start)\b/i.test(prompt)) {
...
export const HuiPlugin = async (_ctx) => {
  ...
  // Intercept user messages to detect /hui commands and natural-language
  ...
  // Inject the reinforcement line into the system prompt when hui is
