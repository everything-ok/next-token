...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hui-freshinstall-'));
}
...
function huiHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'hui-activate.js')),     'hui-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'hui-mode-tracker.js')), 'hui-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'hui-config.js')),       'hui-config.js missing');
    ...
    assert.ok(SETTINGS.hasHuiHook(settings, 'SessionStart', 'hui-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasHuiHook(settings, 'UserPromptSubmit', 'hui-mode-tracker'),
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = huiHookCommands(settings, 'SessionStart', 'hui-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart hui hook, got ${sessStart.length}`);
...
test('uninstall strips hui hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['hui-activate.js', 'hui-mode-tracker.js', 'hui-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining hui-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /hui/, `${ev} still has hui hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = huiHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'hui', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'hui', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- hui-begin -->/, 'SOUL.md missing begin marker');
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'hui', 'SKILL.md'), 'utf8');
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'hui')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- hui-begin -->/, 'hui block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- hui-end -->/, 'hui end marker survived uninstall');
...
test('hui-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'hui-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `hui-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'hui', 'SKILL.md')), 'skill missing via init route');
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/hui-activate.js"',
      marker: 'hui-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/hui-activate.js"',
      marker: 'hui-activate',
    });
...
test('opencode: --force on legacy AGENTS.md preserves user content and takes a backup', () => {
  ...
  const legacyBody = fs.readFileSync(
    path.join(REPO_ROOT, 'src', 'rules', 'hui-activate.md'), 'utf8').trimEnd() + '\n';
  ...
  try {
    ...
    assert.match(after, /<!-- hui-begin -->/, 'fenced block missing after migration');
    assert.match(after, /<!-- hui-end -->/, 'fence end missing after migration');
    // Legacy un-fenced copy must be gone: sentinel appears only inside the fence.
    const beforeFence = after.slice(0, after.indexOf('<!-- hui-begin -->'));
    assert.doesNotMatch(beforeFence, /Respond terse like smart hui/,
...
test('missing claude CLI: reports failure and falls back to standalone hook wiring', () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(configDir, 'hooks', 'hui-activate.js')),
