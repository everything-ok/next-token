...
test('hermes fresh install lands every managed skill under skills/productivity/', () => {
  ...
  try {
    ...
    // hui-compress ships executable scripts — ensure the recursive copy kept them.
    assert.ok(fs.existsSync(path.join(prod, 'hui-compress', 'scripts')), 'hui-compress/scripts/ not copied');
  } finally {
...
test('hermes uninstall removes all installed hui skills (no orphans)', () => {
  const home = freshHome();
  try {
