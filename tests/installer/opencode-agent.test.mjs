...
// Claude Code accepts. Copying agents/huicrew-*.md verbatim into
// ~/.config/opencode/agents/ broke opencode startup with:
//   Configuration is invalid at .../huicrew-reviewer.md
...
const SHIPPED_AGENT_FILES = ['huicrew-investigator.md', 'huicrew-builder.md', 'huicrew-reviewer.md'];
...
function assertNoBareLf(content) {
...
test('strips tools from CRLF frontmatter without changing line endings', () => {
  const src = [
    '---',
    'name: test-agent',
    'tools: [Read, Grep, Bash]',
  ...
  const out = stripOpencodeAgentTools(src);
  assert.doesNotMatch(frontmatter(out), /^tools:/m);
  assert.match(out, /^---\r\nname: test-agent\r\nmodel: haiku\r\n---\r\nbody\r\n$/);
  assertNoBareLf(out);
});
...
// This is the RED-state proof: each `agents/huicrew-*.md` in the repo today
...
test('all shipped huicrew agent files contain offending tools array (RED proof)', () => {
...
test('all shipped huicrew agent files become opencode-safe after transform (GREEN proof)', () => {
  for (const f of SHIPPED_AGENT_FILES) {
    const src = fs.readFileSync(path.join(REPO_ROOT, 'agents', f), 'utf8');
    const out = stripOpencodeAgentTools(src);
    const fm = frontmatter(out);
    ...
    assert.match(fm, /^name: huicrew-/m, `${f}: name field preserved`);
