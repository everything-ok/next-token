...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hui-opencode-'));
}
...
test('opencode fresh install drops plugin, commands, agents, skills, AGENTS.md, opencode.json', () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'hui', 'plugin.js')), 'plugin.js missing');
    ...
    for (const name of ['hui', 'hui-commit', 'hui-review', 'hui-help', 'hui-stats', 'hui-compress', 'huicrew']) {
    ...
    assert.match(agentsBody, /Respond terse like smart hui/);
    // Block must be wrapped in begin/end markers so uninstall can isolate it
    // from user-authored content above and below.
    assert.match(agentsBody, /<!-- hui-begin -->/);
    assert.match(agentsBody, /<!-- hui-end -->/);
    ...
    assert.ok(cfg.plugin.includes('./plugins/hui/plugin.js'), 'plugin entry missing');
  } finally {
...
test('opencode idempotent install does not duplicate plugin entries', () => {
  ...
  try {
    ...
    const matches = cfg.plugin.filter(p => p === './plugins/hui/plugin.js');
    ...
    const sentinelCount = (agentsMd.match(/Respond terse like smart hui/g) || []).length;
...
test('opencode re-install preserves user edits to plugin.js without --force', () => {
  ...
  try {
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'hui', 'plugin.js');
...
test('opencode uninstall strips fenced AGENTS.md block, preserving user prefix and suffix', () => {
  ...
  try {
    ...
    // Sandwich the hui block between user prefix and suffix.
    ...
    assert.doesNotMatch(after, /<!-- hui-begin -->/, 'hui block should be stripped');
    assert.doesNotMatch(after, /<!-- hui-end -->/, 'hui end marker should be stripped');
    assert.doesNotMatch(after, /Respond terse like smart hui/, 'hui body should be stripped');
...
test('opencode uninstall removes plugin dir, command/agent/skill files, prunes opencode.json', () => {
  ...
  try {
    ...
    const ocDir = path.join(xdg, 'opencode');
    assert.equal(fs.existsSync(path.join(ocDir, 'plugins', 'hui')), false, 'plugin dir survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'commands', 'hui.md')), false, 'hui.md command survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'agents', 'huicrew-builder.md')), false, 'huicrew agent survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'skills', 'hui')), false, 'hui skill dir survived');
    ...
    if (fs.existsSync(path.join(ocDir, 'opencode.json'))) {
      const cfg = JSON.parse(fs.readFileSync(path.join(ocDir, 'opencode.json'), 'utf8'));
      const stillHasPlugin = Array.isArray(cfg.plugin) && cfg.plugin.includes('./plugins/hui/plugin.js');
...
test('opencode plugin handles /hui ultra, stop hui, and session init via real hooks', async () => {
  const xdg = freshTmpDir();
  const shimDir = shimOpencode();
  const origDefault = process.env.HUI_DEFAULT_MODE;
  try {
    const env = { ...process.env, XDG_CONFIG_HOME: xdg, PATH: pathWith(shimDir), NO_COLOR: '1' };
    const r = runInstaller(['--only', 'opencode'], env);
    assert.notEqual(r.status, 2);
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'hui', 'plugin.js');
    const flagPath = path.join(xdg, 'opencode', '.hui-active');
    ...
    // any ambient user/repo-local hui config.
    process.env.XDG_CONFIG_HOME = xdg;
    process.env.HUI_DEFAULT_MODE = 'full';

    const mod = await import(pathToFileURL(pluginPath).href);
    const factory = mod.default || mod.HuiPlugin;
    ...
    // opencode expands "/hui <level>" into the command template before
    ...
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: '"/hui lite"\n' }] });
    ...
    assert.match(sys1.system[0], /HUI MODE ACTIVE \(ultra\)/);

    // Natural-language deactivation removes the flag.
    await handlers['chat.message']({}, { parts: [{ type: 'text', text: 'stop hui please' }] });
    ...
    if (origDefault === undefined) delete process.env.HUI_DEFAULT_MODE;
    else process.env.HUI_DEFAULT_MODE = origDefault;
    fs.rmSync(xdg, { recursive: true, force: true });
    fs.rmSync(shimDir, { recursive: true, force: true });
  }
});
