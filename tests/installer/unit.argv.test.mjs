...
test('--list prints provider matrix', () => {
  const r = run('--list');
  assert.equal(r.status, 0);
  assert.match(r.stdout, /hui provider matrix/);
...
test('--only with unknown agent id exits 2', () => {
  ...
  assert.match(r.stderr, /hui --list/);
});
...
test('--config-dir expands ~ to home directory', async () => {
  ...
  // match even when the hui plugin is already installed.
  ...
  if (/Claude Code detected/.test(r.stdout)) {
    ...
    assert.match(normalized, new RegExp(homedir().replace(/\\/g, '/').replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '/' + suffix));
  }
});
...
test('bare --with-mcp-shrink (no upstream) exits 2 with hint', () => {
  ...
  // that crashed on every Claude Code startup. hui-shrink is a proxy and
...
test('--with-mcp-shrink="<cmd>" registers wrapping that upstream', () => {
  ...
  if (/would run: claude mcp add hui-shrink/.test(r.stdout)) {
    assert.match(r.stdout, /claude mcp add hui-shrink .* npx -y hui-shrink npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});
...
test('--with-mcp-shrink "<cmd>" (space-separated) also accepted', () => {
  ...
  assert.equal(r.status, 0);
  if (/would run: claude mcp add hui-shrink/.test(r.stdout)) {
    assert.match(r.stdout, /hui-shrink npx @modelcontextprotocol\/server-filesystem \/tmp/);
  }
});
...
test('--all does NOT auto-enable mcp-shrink (no sensible default upstream)', () => {
  ...
  assert.doesNotMatch(r.stdout, /wiring hui-shrink MCP proxy/);
});
