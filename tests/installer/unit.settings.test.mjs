...
test('addCommandHook is idempotent on substring marker', () => {
  const s = {};
  const a = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/abs/path/hui-activate.js', marker: 'hui-activate' });
  const b = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/different/abs/path/hui-activate.js', marker: 'hui-activate' });
  assert.equal(a, true);
  assert.equal(b, false);
  assert.equal(s.hooks.SessionStart.length, 1);
});

test('hasHuiHook detects via substring', () => {
  const s = { hooks: { SessionStart: [{ hooks: [{ type: 'command', command: 'node /x/hui-activate.js' }] }] } };
  assert.equal(SETTINGS.hasHuiHook(s, 'SessionStart', 'hui-activate'), true);
  assert.equal(SETTINGS.hasHuiHook(s, 'SessionStart', 'gsd'), false);
  assert.equal(SETTINGS.hasHuiHook(s, 'UserPromptSubmit'), false);
});

test('removeHuiHooks tolerates malformed hook event values without throwing', () => {
  ...
  let removed;
  assert.doesNotThrow(() => { removed = SETTINGS.removeHuiHooks(s); });
  assert.equal(removed, 0);
  assert.equal(s.hooks, undefined);
});

test('removeHuiHooks strips managed scripts and cleans empties', () => {
  const s = {
    hooks: {
      SessionStart: [
        { hooks: [{ type: 'command', command: 'node /x/hooks/hui-activate.js' }] },
        { hooks: [{ type: 'command', command: 'other' }] },
      ],
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: '"/usr/bin/node" "/x/hooks/hui-mode-tracker.js"' }] }],
    },
  };
  const removed = SETTINGS.removeHuiHooks(s);
  assert.equal(removed, 2);
  assert.equal(s.hooks.SessionStart.length, 1);
  assert.equal(s.hooks.UserPromptSubmit, undefined);
});

test('removeHuiHooks leaves user hooks that merely mention hui (issue #593)', () => {
  const s = {
    hooks: {
      PreToolUse: [
        // Path contains the word "hui" but targets a user-authored script.
        { hooks: [{ type: 'command', command: 'node /Users/me/Projects/hui-notes/my-hook.js' }] },
        // Basename is a superstring of a managed name — still not ours.
        { hooks: [{ type: 'command', command: 'node /x/myhui-activate.js' }] },
      ],
      SessionStart: [
        { hooks: [{ type: 'command', command: '"/usr/bin/node" "/x/hooks/hui-activate.js"' }] },
      ],
    },
  };
  const removed = SETTINGS.removeHuiHooks(s);
  assert.equal(removed, 1, 'only the managed SessionStart hook should be removed');
  assert.equal(s.hooks.PreToolUse.length, 2, 'user hooks mentioning hui must survive uninstall');
  assert.equal(s.hooks.SessionStart, undefined);
});

test('removeHuiHooks removes the Windows statusline-stats wiring (hui-stats.js / .ps1)', () => {
  const s = {
    hooks: {
      Stop: [{ hooks: [{ type: 'command', command: '"C:\\Program Files\\nodejs\\node.exe" "C:\\Users\\me\\.claude\\hooks\\hui-stats.js"' }] }],
    },
  };
  const removed = SETTINGS.removeHuiHooks(s);
...
test('rewriteLegacyManagedHookCommands rewrites bare-node managed scripts', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node /abs/hooks/hui-activate.js' },
  ...
  assert.equal(n, 1);
  assert.match(s.hooks.SessionStart[0].hooks[0].command, /"\/usr\/local\/bin\/node" "\/abs\/hooks\/hui-activate\.js"/);
...
test('rewriteLegacyManagedHookCommands ignores already-absolute node commands', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/usr/local/bin/node" "/abs/hooks/hui-activate.js"' },
      ] }],
    },
  };
...
test('pruneOrphanedManagedHooks removes managed hook whose target is missing (absolute-node)', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/opt/node/bin/node" "/no/such/dir/hui-activate.js"' },
      ] }],
    },
  };
  ...
  const s = {
    hooks: {
      UserPromptSubmit: [{ hooks: [
        { type: 'command', command: 'node /no/such/dir/hui-mode-tracker.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks keeps managed hook whose target exists', () => {
  ...
  const script = path.join(dir, 'hui-activate.js');
...
test('pruneOrphanedManagedHooks resolves relative target against configDir', () => {
  ...
  // hooks/hui-activate.js intentionally NOT created → missing
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node hooks/hui-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, dir);
...
test('pruneOrphanedManagedHooks does NOT match a user script whose name merely contains a managed basename', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        // basename is "myhui-activate.js" — not an exact managed basename
        { type: 'command', command: 'node /no/such/dir/myhui-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks drops orphaned managed statusLine', () => {
  const s = {
    statusLine: { type: 'command', command: 'bash /no/such/dir/hui-statusline.sh' },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
  assert.equal(removed, 1);
