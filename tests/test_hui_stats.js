#!/usr/bin/env node
// Tests for /hui-stats — direct script invocation and via mode tracker.
// Run: node tests/test_hui_stats.js
...
test('shows full-mode savings estimate when flag is full', (tmp) => {
  ...
  assert.match(out, /Est\. without hui:\s+1,000/);
...
test('skips estimate for non-full modes', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.hui-active'), 'ultra');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('mode tracker handles /hui-stats with decision block', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 100 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    ...
    input: JSON.stringify({ prompt: '/hui-stats', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /Hui Stats/);
  assert.match(parsed.reason, /Output tokens:\s+100/);
});

test('mode tracker preserves hui flag when /hui-stats fires', (tmp) => {
  const sess = makeSession(tmp, [
...
test('priceForModel matches by prefix across point releases', () => {
  const { priceForModel } = require(path.join(ROOT, 'src', 'hooks', 'hui-stats.js'));
...
test('formatStats handles empty session gracefully', () => {
  const { formatStats } = require(path.join(ROOT, 'src', 'hooks', 'hui-stats.js'));
...
test('--share prints single-line tweetable summary', (tmp) => {
  ...
  assert.match(out, /^🪨 Saved 650 output tokens \(~\$0\.009[78]\) across 1 turns this session — hui\.sh$/m);
});
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.hui-active'), 'lite');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess, '--share'], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.match(out, /^🪨 1 turns, 200 output tokens this session — hui\.sh$/m);
});
...
test('appends to lifetime history on each run', (tmp) => {
  ...
  const histPath = path.join(claudeDir, '.hui-history.jsonl');
...
test('skips pairs where compressed is not actually smaller', (tmp) => {
  const { findCompressedPairs } = require(path.join(ROOT, 'src', 'hooks', 'hui-stats.js'));
...
test('writes statusline suffix file after a stats run', (tmp) => {
  ...
  const suffixPath = path.join(claudeDir, '.hui-statusline-suffix');
...
test('humanizeTokens formats small/medium/large correctly', () => {
  const { humanizeTokens } = require(path.join(ROOT, 'src', 'hooks', 'hui-stats.js'));
...
test('statusline.sh appends savings when HUI_STATUSLINE_SAVINGS=1', (tmp) => {
  if (process.platform === 'win32') return; // bash test
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.hui-active'), 'full');
  fs.writeFileSync(path.join(claudeDir, '.hui-statusline-suffix'), '⛏ 2.8k');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'hui-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HUI_STATUSLINE_SAVINGS: '1' },
  });
  assert.match(out, /\[HUI\]/);
...
test('statusline.sh renders savings by default when env var is unset', (tmp) => {
  ...
  delete env.HUI_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'hui-statusline.sh')], {
    encoding: 'utf8', env,
  });
  assert.match(out, /\[HUI\]/);
...
test('statusline.sh omits savings when HUI_STATUSLINE_SAVINGS=0', (tmp) => {
  if (process.platform === 'win32') return;
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.hui-active'), 'full');
  ...
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'hui-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HUI_STATUSLINE_SAVINGS: '0' },
  });
  assert.match(out, /\[HUI\]/);
...
test('statusline.sh omits savings when suffix file is missing (fresh install)', (tmp) => {
  ...
  // /hui-stats has run. Default-on must NOT fabricate a number.
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.HUI_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'hui-statusline.sh')], {
    encoding: 'utf8', env,
...
test('statusline.sh strips control bytes from suffix', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.hui-statusline-suffix'), '\x1b[31mEVIL');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'hui-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HUI_STATUSLINE_SAVINGS: '1' },
  });
...
test('appendFlag is symlink-safe (refuses symlinked target)', (tmp) => {
  ...
  const { appendFlag } = require(path.join(ROOT, 'src', 'hooks', 'hui-config.js'));
...
test('mode tracker forwards --share to stats script', (tmp) => {
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HOME: tmp },
    input: JSON.stringify({ prompt: '/hui-stats --share', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
...
test('--all lifetime output labels the % as output reduction, not usage', (tmp) => {
  ...
  fs.writeFileSync(
    path.join(claudeDir, '.hui-history.jsonl'),
...
// via the .hui-mode-log.jsonl transition log — never to whatever mode the
...
test('attributes tokens to the mode active when each message happened (#601)', (tmp) => {
  ...
  // 300 verbose tokens BEFORE hui was activated, 350 after.
  const sess = makeSession(tmp, [
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.hui-mode-log.jsonl'),
  ...
  assert.match(out, /hui off:\s+300 tokens \(no benchmark estimate\)/);
  ...
  const hist = fs.readFileSync(path.join(claudeDir, '.hui-history.jsonl'), 'utf8')
    .split('\n').filter(Boolean).map(l => JSON.parse(l));
  assert.strictEqual(hist[hist.length - 1].est_saved_tokens, 650);
});

test('credits hui spans even after mode is turned off mid-session (#601)', (tmp) => {
  const now = Date.now();
  const iso = (minAgo) => new Date(now - minAgo * 60_000).toISOString();
  const sess = makeSession(tmp, [
  ...
  // No .hui-active flag — hui is off at stats time. The old behavior
  // printed "Hui not active this session." and logged zero savings.
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.doesNotMatch(out, /Hui not active this session/);
  assert.match(out, /full:\s+350 tokens \(est\. 650 saved\)/);
  assert.match(out, /Est\. tokens saved:\s+650\b/);
});
...
test('excludes tokens that predate a mid-session flag write with no log (#601)', (tmp) => {
  ...
  assert.doesNotMatch(out, /Est\. without hui/);
});
