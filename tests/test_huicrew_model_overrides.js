#!/usr/bin/env node
// Tests for src/hooks/huicrew-model-overrides.js
// Run: node tests/test_huicrew_model_overrides.js
...
test('preserves all other frontmatter lines', () => {
  ...
  assert.ok(out.includes('name: huicrew-reviewer'), 'name line lost');
...
function withTmpPlugin(fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'hui-override-test-'));
...
test('replaces reviewer model when HUICREW_REVIEWER_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: 'sonnet' });
...
test('HUICREW_REVIEWER_MODEL patches reviewer model', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: 'opus' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), 'utf8');
    assert.ok(out.includes('model: opus'), 'canonical reviewer model not patched');
  });
});

test('replaces investigator model when HUICREW_INVESTIGATOR_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-investigator.md'), INVESTIGATOR_FM, 'utf8');
    applyOverrides(root, { HUICREW_INVESTIGATOR_MODEL: 'opus' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-investigator.md'), 'utf8');
    assert.ok(out.includes('model: opus'), 'investigator model not patched');
  });
});

test('inserts builder model when HUICREW_BUILDER_MODEL set and no model line', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-builder.md'), BUILDER_FM, 'utf8');
    applyOverrides(root, { HUICREW_BUILDER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-builder.md'), 'utf8');
...
test('blank env var is no-op', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: '' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), 'utf8');
...
test('env var with newline in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: 'so\nnnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), 'utf8');
...
test('env var with control character in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: 'son\x00net' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), 'utf8');
...
test('missing agents dir (non-plugin layout) is silent no-op', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'hui-nolayout-'));
  try {
    assert.doesNotThrow(() => {
      applyOverrides(tmp, { HUICREW_REVIEWER_MODEL: 'sonnet' });
    });
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
...
test('body content preserved after model patch', () => {
  withTmpPlugin((root, agentsDir) => {
    ...
    fs.writeFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), content, 'utf8');
    applyOverrides(root, { HUICREW_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'huicrew-reviewer.md'), 'utf8');
