...
//   env HUI_DEFAULT_MODE → repo-local (.hui/config.json or .hui.json,
...
// own ~/.config/hui/config.json.
const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'hui-userhome-'));
process.env.XDG_CONFIG_HOME = tmpHome;
delete process.env.HUI_DEFAULT_MODE;

const { getDefaultMode, findRepoConfigPath } = require('../src/hooks/hui-config');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'hui-repocfg-'));
  const origCwd = process.cwd();
  const origEnv = process.env.HUI_DEFAULT_MODE;
  try {
    fn(tmpBase);
    passed++;
    console.log(`  ✓ ${name}`);
    ...
    if (origEnv === undefined) delete process.env.HUI_DEFAULT_MODE;
    else process.env.HUI_DEFAULT_MODE = origEnv;
...
test('reads .hui/config.json in cwd', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.hui'));
  fs.writeFileSync(path.join(tmp, '.hui', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'lite');
});

test('reads .hui.json in cwd', (tmp) => {
  fs.writeFileSync(path.join(tmp, '.hui.json'),
    JSON.stringify({ defaultMode: 'ultra' }));
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'ultra');
});

test('.hui/config.json wins over .hui.json at same level', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.hui'));
  fs.writeFileSync(path.join(tmp, '.hui', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  fs.writeFileSync(path.join(tmp, '.hui.json'),
...
test('env var beats repo-local config', (tmp) => {
  ...
  fs.writeFileSync(path.join(tmp, '.hui', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  process.chdir(tmp);
  process.env.HUI_DEFAULT_MODE = 'ultra';
  assert.strictEqual(getDefaultMode(), 'ultra');
});

test('repo-local config beats user config', (tmp) => {
  // user config at XDG_CONFIG_HOME points to 'commit'
  fs.mkdirSync(path.join(tmpHome, 'hui'), { recursive: true });
  fs.writeFileSync(path.join(tmpHome, 'hui', 'config.json'),
  ...
  try {
    ...
    fs.rmSync(path.join(tmpHome, 'hui'), { recursive: true, force: true });
  }
});
...
test('malformed JSON in repo config falls through to default', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.hui'));
  fs.writeFileSync(path.join(tmp, '.hui', 'config.json'), '{ not json');
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'full');
});

test('refuses symlinked .hui.json (symmetric with readFlag policy)', (tmp) => {
  const real = path.join(tmp, 'real-config.json');
  fs.writeFileSync(real, JSON.stringify({ defaultMode: 'ultra' }));
  try {
    fs.symlinkSync(real, path.join(tmp, '.hui.json'));
  } catch (e) {
